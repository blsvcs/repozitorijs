...
TOPICS: list[tuple[str, list[str]]] = [
    ("Kredīti un parādi", ["kredīt", "aizdev", "parād", "piedziņ", "procent", "līgumsod", "galvojum", "parāda atmaksa"]),
    ("Darba tiesības", ["darba līgum", "darba samaks", "atlaišan", "uzteikum", "darba devēj", "darbiniek", "disciplinār", "darba tiesisk"]),
...
def classify(text: str, process: str = "", material_type: str = "") -> tuple[str, int, list[str]]:
    ...
    if best_score == 0:
        if "krimināl" in normalize(process):
            return "Krimināllietas", 1, ["process"]
        if "administratīv" in normalize(process):
            return "Administratīvās lietas", 1, ["process"]
        if "civil" in normalize(process):
            return "Civillietas", 1, ["process"]
